...
export default function PlantPot({
  ...
  if (!isLinked) {
    return (
      <div aria-hidden="true" className="flex flex-shrink-0 self-end items-end">
        <PlantPotGraphic label={null} interactive={false} />
      </div>
    );
  }
  ...
  return (
    <Link
      ...
      className="theme-focus-ring group flex flex-shrink-0 self-end items-end"
    >
      <PlantPotGraphic label={resolvedLabel!} interactive />
    </Link>
  );
}
